var ye_dialog = {
    ...
    setAbsHeight:function (obj_id){
	var w_h = $(window).height();
	var s_h = $(window).scrollTop(); 
	var o_h = $('#'+obj_id).height(); 
	h = parseInt(w_h - o_h)/2  + s_h;
	$('#'+obj_id).css("top",h);
    },
    setAbsLeft:function (obj_id){
	var w_w = $(window).width();
	var o_w = $('#'+obj_id).width(); 
	w = parseInt(w_w)/2;
	$('#'+obj_id).css("left",w);
    }
...
var ye_msg = {
    ...
    open: function(e, d, c, b){//(e:html,d:timer,c:type(int),b:callback)
        ...
        this.Counter++;
        var a = '<div id="ye_msg_' + this.Counter + '" class="ye_msg_window"><div class="ye_msg_wrap"><p>' + e + "</p></div>"+' <div class="sbg"></div>'+'</div>';

        $("body").append(a);
	    ye_dialog.setAbsHeight('ye_msg_' + this.Counter);
		ye_dialog.setAbsLeft('ye_msg_' + this.Counter);
        ...
        if (d != undefined && d != 0) {
            $("#ye_msg_" + this.Counter + " > .ye_msg_wrap").append('<div class="ye_msg_autoclose"><p>(' + d + "\u79d2\u540e\u8fd4\u56de)</p></div>");
